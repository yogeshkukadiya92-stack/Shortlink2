...
function buildGoalMarkup(link) {
  ...
  return `<span class="analytics-tag strong ${achieved ? "success" : ""}">${achieved ? `Goal achieved Â· ${clicks}/${goal}` : `Goal ${goal} Â· ${progress}% reached`}</span>`;
}
        ...
        <article class="mini-card inset-card submission-card">
          <div class="surface-header">
            <div>
              ...
              <p>${escapeHtml(submission.meta?.city || "Unknown city")}, ${escapeHtml(submission.meta?.country || "Unknown country")} â€¢ ${escapeHtml(submission.meta?.device || "Web")} â€¢ ${escapeHtml(submission.meta?.browser || "Unknown browser")}</p>
            </div>
          ...
          ${analytics.links.length ? analytics.links.map((link, index) => `
            <article class="mini-card inset-card analytics-report-card leaderboard-card">
              <div class="analytics-report-topline">
                ...
                <span class="chip-link">${link.totalClicks} clicks Â· ${link.uniqueClicks || 0} unique Â· ${link.repeatClicks || 0} repeat</span>
              </div>
  ...
  return clicks.map((click) => `
    <div class="admin-row analytics-row ${compact ? "compact" : ""}">
      <div class="admin-main">
        ...
        <span>${escapeHtml(click.country || "Unknown")} â€¢ ${escapeHtml(click.city || "Unknown")}</span>
        <span>${escapeHtml(click.deviceType || "Unknown")} â€¢ ${escapeHtml(click.platform || "Unknown")} â€¢ ${escapeHtml(click.browser || "Unknown")}</span>
      </div>
